...
(function() {
  ...
  module.exports = function(options, model) {
    ...
    })).use(require('./middleware/publishId')()).use(require('./middleware/ironsModel')()).use(require('./middleware/passportSerializers')()).use(passport.initialize()).use(passport.session()).post('/register', require('./controllers/acceptRegistration')(config)).post('/login', passport.authenticate('local', config.get('passport'))).get('/logout', function(req, res) {
      req.logout();
      return res.redirect('/');
    });
  };
